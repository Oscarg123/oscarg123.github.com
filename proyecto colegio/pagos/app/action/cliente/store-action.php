<?php
	if(isset($_SESSION["user_id"])){
		if(count($_POST)>0){
				...
				$sqlByCel = ClienteData::getBy('telefono',Database::cleanChain($_POST["telefono"]));

				if (count($sqlByCel)==null or count($sqlByCel)==0) {
					$cliente->add();
					Core::alert('success','¡Bien hecho!','Cliente agregado exitosamente!');
					Core::redir("cliente/create");
				}else{
					Core::alert('warning','¡Error!','El numero de telefóno ingresado ya existe!');
					Core::redir("cliente/create");
				}
